...
/**
 * contract Validator {
 *  .   string[] whitelistedCountries
 *  .   string[] blacklistedCountries
 *
 *  .
 *
 *  .
 *
 *     function addCountry( string country, bool flagIfWhite)
 *  .  function remove Country (string country, bool isWhite)
 *  .  function resetAll() - // clean white, black & attributes
 *
 *  .  function validateUser(address){
 *  .    did = DIDSC.getUserDID(address)
 *
 *
 *  .    _checkUserCountry
 *  .
 ...
 *  .    if whiteSizee + blackSize == 0 {
 *  .     return true;
 * } else {
 *     for all whiteCountries look for {
 *  .        if keccak256( userAddress + country) == userDID.countryHash
 *  .             return true
 *
 *  .  for all blackCountries look for {
 *  .        if keccak256( userAddress + country) == userDID.countryHash
 *  .             return false
 * }
 *  . return false
